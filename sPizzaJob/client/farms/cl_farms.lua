...
local mainMenu = RageUI.CreateMenu('~y~Réc~g~olte', '~g~Pizza')
...
function StartRecolte()
    ...
    while recoltepossible do
        Citizen.Wait(2000)
        TriggerServerEvent('recoltegigapizzafroid')
    end
    else
        recoltepossible = false
    end
...
--- Traitement de la giga pizza ---

local mainMenu = RageUI.CreateMenu('~y~Trait~g~ement', '~g~Pizza')
...
function StartTraitement()
    ...
    while traitementpossible do
            Citizen.Wait(2000)
            TriggerServerEvent('traitementgigapizzafroid')
    end
    else
        traitementpossible = false
    end
